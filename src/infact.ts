...
export class Infact<T extends TInfactClassMeta = TInfactClassMeta> {
    ...
    async get<T = unknown>(classConstructor: TClassConstructor<T>, provide?: TProvideRegistry, hierarchy?: string[]): Promise<T> {
        hierarchy = (hierarchy || [])
        ...
        if (mergedProvide[instanceKey]) {
            return getProvidedValue(mergedProvide[instanceKey]) as Promise<T>
        }
        ...
        hierarchy.pop()
        return (this.registry[instanceKey] || globalRegistry[instanceKey]) as Promise<T>
    }
}
