...
```bash
pnpm test          # run tests (serial via --runInBand)
pnpm test:cov      # run tests with coverage
pnpm build         # rollup → dist/ (ESM, CJS, .d.ts)
pnpm lint          # eslint --fix on src/**/*.ts
```
...
- `src/tests/` — Jest specs and fixture classes (artifacts)
...
Compile-time replacements: `process.env.NODE_ENV` → `"production"`, all `__DYE_*__` color constants inlined as ANSI strings (no runtime `@prostojs/dye` dependency in output). Jest config mirrors these globals for tests.
